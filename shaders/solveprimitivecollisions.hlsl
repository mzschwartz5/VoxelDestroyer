 ...
 */
void applyFriction(inout Particle particle, Particle oldParticle, float3 colliderNormal, float friction) {
    float3 deltaPos = particle.position - oldParticle.position;
    float3 deltaTangent = deltaPos - dot(deltaPos, colliderNormal) * colliderNormal;
    particle.position -= deltaTangent * friction;
}
...
float3 solveSphereCollision(float4x4 wMatrix, inout Particle particle, float particleRadius)
{
    ...
    float3 toParticle = particle.position - sphereCenter;
    ...
    particle.position = sphereCenter + norm * (sphereRadius + particleRadius);
    return norm;
}

float3 solveBoxCollision(float4x4 wMatrix, float4x4 invWMatrix, inout Particle particle, float particleRadius)
{
    // In the box's object space, the box is axis-aligned and easier to work with
    float3 localPos = mul(invWMatrix, float4(particle.position, 1.0f)).xyz;
    ...
    particle.position = mul(wMatrix, float4(adjustedLocalPos, 1.0f)).xyz;
...
float3 solvePlaneCollision(float4x4 wMatrix, inout Particle particle, float particleRadius)
{
    ...
    float dist = dot(particle.position - planePos, planeNormal) - particleRadius;
    ...
    if (isInfinite == 0.0f) // 0 => finite, non-zero => infinite
    {
        ...
        float3 toPoint = particle.position - planePos;
    ...
    particle.position -= dist * planeNormal;
    return planeNormal;
}

float3 solveCylinderCollision(float4x4 wMatrix, float4x4 invWMatrix, inout Particle particle, float particleRadius)
{
...
float3 solveCapsuleCollision(float4x4 wMatrix, float4x4 invWMatrix, inout Particle particle, float particleRadius)
{
...
void main(uint3 globalThreadId : SV_DispatchThreadID)
{
    ...
    for (int i = 0; i < numColliders; i++) {
        ...
        applyFriction(particle, oldParticle, colliderNormal, friction);
    }

    particles[globalThreadId.x] = particle;
}