...
public:
    ...
    void updateFaceConstraintsFromPaint(const ComPtr<ID3D11UnorderedAccessView>& paintDeltaUAV) {
...
private:
    ...
    void bind() override
    {
        ID3D11UnorderedAccessView* uavs[] = { positionsUAV.Get(), faceConstraintUAVs[activeConstraintAxis].Get(), isSurfaceUAV.Get(), paintDeltaUAV.Get() };
    ...
    void initializeBuffers(const std::array<std::vector<FaceConstraint>, 3>& constraints) {
        ...
        std::array<FaceConstraintsCB, 3> faceConstraintsCBData = {
            FaceConstraintsCB{{1, 3, 5, 7}, {0, 2, 4, 6}, static_cast<uint>(constraints[0].size()), 0, 3, 0},
            FaceConstraintsCB{{2, 3, 6, 7}, {0, 1, 4, 5}, static_cast<uint>(constraints[1].size()), 1, 4, 0},
            FaceConstraintsCB{{4, 5, 6, 7}, {0, 1, 2, 3}, static_cast<uint>(constraints[2].size()), 2, 5, 0}
        };        
